---
layout: base
---

# Mechanics (Physics)

## [Classic](Classic/Classic.md)
Describing the motion of objects such as projectiles, parts of machinery, spacecraft, planets, stars, galaxies, deformable solids, fluids, macromolecules and other objects. 
## [Fluid & Gas & Heat & statistic](Fluid_Gas_Heat_statistic/Fluid_Gas_Heat_statistic.md)
Describing heat transfer, thermodynamics, fluid mechanics, and combustion. Discussing the overall impact rather than the motion of a single object
## [SHM](SHM/SHM.md)
SHM is a special type of periodic motion an object experiences by means of a restoring force whose magnitude is directly proportional to the distance of the object from an equilibrium position and acts towards the equilibrium position. 
## [Modern](Modern/Modern.md)
Relativity and Fundamental interaction, Nuclear physics.
## Math tools

1. [Vector](Math_tools/Vector.md)
1. [Mathematical formula](Math_tools/Mathematical_formula.md)
1. [Physics notations](Math_tools/physics_notations.md)
1. [Constants](Math_tools/Constant.md)
1. [Greek alphabet](Math_tools/Greek_alphabet.md)

## General Physics

1. [Force & motion](General_Physics/force_and_motion/force_and_motion.md)
1. [Heat](General_Physics/heat/heat.md)
1. [Wave](General_Physics/wave/wave.md)
1. [Light](General_Physics/Light/Light.md)
1. [Electricity](General_Physics/Electricity/Electricity.md)

## formulas

1. [Mechanics](formulas/Mechanics/Mechanics.md)
1. [Energy](formulas/Energy/Energy.md)

> The following tool demonstrates that simple physics formulas use the same common mathematical relationships. Both files must be downloaded to the same folder to use.

* [formula tool box](formulas/ME_tool_box/Mechanics.py) & [mathematical relationships](formulas/ME_tool_box/number_relation.py)